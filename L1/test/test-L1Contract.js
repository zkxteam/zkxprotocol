...
async function preapreToAdd(asset, zkxContract = L1ZKXContract, starknetMock = starknetCoreMock, assetAddress = L2_ASSET_ADDRESS) {
  const payload = [
    INDEX.ADD_ASSET,
    asset.ticker,
    asset.collateralID
  ];
  await starknetMock.addL2ToL1Message(assetAddress, zkxContract.address, payload);
}

async function addAsset(asset, zkxContract = L1ZKXContract, starknetMock = starknetCoreMock, assetAddress = L2_ASSET_ADDRESS) {
...
async function prepareToRemove(asset, zkxContract = L1ZKXContract, starknetMock = starknetCoreMock, assetAddress = L2_ASSET_ADDRESS) {
  const payload = [
    INDEX.REMOVE_ASSET,
    asset.ticker,
    asset.collateralID
...
async function prepareToWithdraw(asset, recipient, L2Address, amount, requestID) {
  const payload = [
    INDEX.WITHDRAWAL,
    recipient.address,
    asset.ticker,
  ...
  await starknetCoreMock.addL2ToL1Message(L2Address, L1ZKXContract.address, payload);
}
...
describe('Asset management', function () {
  ...
  it("Add ETH", async function () {
    ...
    // When
    await L1ZKXContract.updateAssetListInL1(ETH_ASSET.ticker, ETH_ASSET.collateralID);
  ...
  it("Add ZKXToken", async function () {
    ...
    // When
    await L1ZKXContract.updateAssetListInL1(ZKX_ASSET.ticker, ZKX_ASSET.collateralID);
    await L1ZKXContract.setTokenContractAddress(ZKX_ASSET.ticker, ZKXToken.address);
    ...
    expect(await L1ZKXContract.tokenContractAddress(ZKX_ASSET.ticker)).to.be.eq(ZKXToken.address);
  })
  ...
  it("Add ETH & ZKXToken, then remove ETH", async function () {
    ...
    // When
    await L1ZKXContract.removeAssetFromList(ETH_ASSET.ticker, ETH_ASSET.collateralID);
  ...
  it("Add ETH & ZKXToken, then remove ZKXToken", async function () {
    ...
    // When
    await L1ZKXContract.removeAssetFromList(ZKX_ASSET.ticker, ZKX_ASSET.collateralID);
  ...
  it("Add 3 assets, then remove first", async function () {
    ...
    await L1ZKXContract.removeAssetFromList(ASSET_1.ticker, ASSET_1.collateralID);
  ...
  it("Add 3 assets, then remove middle", async function () {
    ...
    await L1ZKXContract.removeAssetFromList(ASSET_2.ticker, ASSET_2.collateralID);
  ...
  it("Add 3 assets, then remove last", async function () {
    ...
    await L1ZKXContract.removeAssetFromList(ASSET_3.ticker, ASSET_3.collateralID);
...
describe('Deposits', function () {
  ...
  it("Successful ETH deposit", async function () {
    ...
    await expect(aliceContract.depositEthToL1(ALICE_L2_ADDRESS, { value: ONE_ETH }))
  ...
  it('Deposit and then withdraw ETH', async function () {
    ...
    const requestID = 42;
    await prepareToWithdraw(ETH_ASSET, alice, ALICE_L2_ADDRESS, amount, requestID);
    await starknetCoreMock.resetCounters();

    // Rogue can't withdraw Alice's funds
    const rogueContract = L1ZKXContract.connect(rogue)
    await expect(
      rogueContract.withdrawEth(rogue.address, ROGUE_L2_ADDRESS, amount, requestID)
    ).to.be.revertedWith('INVALID_MESSAGE_TO_CONSUME')
    await expect(
      rogueContract.withdrawEth(rogue.address, ALICE_L2_ADDRESS, amount, requestID)
    ).to.be.revertedWith('INVALID_MESSAGE_TO_CONSUME')
    
    // Alice successfully withdraws funds
    await aliceContract.withdrawEth(alice.address, ALICE_L2_ADDRESS, amount, requestID);
  ...
  it("Not possible to deposit Tokens before asset added", async function () {
    ...
    await expect(
      aliceContract.depositToL1(ALICE_L2_ADDRESS, ZKX_ASSET.ticker, 100 * TOKEN_UNIT)
  ...
  it("Not possible to deposit more Tokens than user has", async function () {
    ...
    await expect(
      aliceContract.depositToL1(ALICE_L2_ADDRESS, ZKX_ASSET.ticker, 101 * TOKEN_UNIT)
    ).to.be.reverted;
  });
  ...
  it("Successful Token deposit", async function () {
    ...
    await L1ZKXContract.setTokenContractAddress(ZKX_ASSET.ticker, ZKXToken.address);
    await ZKXToken.mint(alice.address, 100 * TOKEN_UNIT);
    await ZKXToken.connect(alice).approve(L1ZKXContract.address, 100 * TOKEN_UNIT)
    await starknetCoreMock.resetCounters();

    await expect(aliceContract.depositToL1(ALICE_L2_ADDRESS, ZKX_ASSET.ticker, 100 * TOKEN_UNIT))
      .to.emit(L1ZKXContract, "LogDeposit")
  ...
  it('Deposit and then withdraw tokens', async function () {
    ...
    const requestID = 42;
    await prepareToWithdraw(ZKX_ASSET, alice, ALICE_L2_ADDRESS, amount, requestID);
    await starknetCoreMock.resetCounters();

    // Rogue can't withdraw Alice's funds
    await expect(
      rogueContract.withdraw(rogue.address, ROGUE_L2_ADDRESS, ZKX_ASSET.ticker, amount, requestID)
    ).to.be.revertedWith('INVALID_MESSAGE_TO_CONSUME')

    await expect(
      rogueContract.withdraw(rogue.address, ALICE_L2_ADDRESS, ZKX_ASSET.ticker, amount, requestID)
    ).to.be.revertedWith('INVALID_MESSAGE_TO_CONSUME')
    
    // Alice successfully withdraws funds
    await aliceContract.withdraw(alice.address, ALICE_L2_ADDRESS, ZKX_ASSET.ticker, amount, requestID);
    // Withdrawal should consume 1 message from L2
    expect(await starknetCoreMock.invokedConsumeMessageFromL2Count()).to.be.eq(1);
  ...
  it('Multiple token deposits', async function () {
    ...
    await expect(aliceContract.depositToL1(ALICE_L2_ADDRESS, ZKX_ASSET.ticker, 301 * TOKEN_UNIT)).to.be.reverted;
    
    // This deposit succeeds, after tx Alice has 200 tokens left
    await aliceContract.depositToL1(ALICE_L2_ADDRESS, ZKX_ASSET.ticker, 100 * TOKEN_UNIT);
    ...
    await expect(aliceContract.depositToL1(ALICE_L2_ADDRESS, ZKX_ASSET.ticker, 1 * TOKEN_UNIT)).to.be.reverted;
  });
