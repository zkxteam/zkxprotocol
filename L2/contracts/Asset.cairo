...
func removeAsset{
    ...
    asset.write(id = id, value = Asset(ticker = 0, short_name = 0, tradable = 0, collateral = 0, tick_size = 0, 
...
# @param tradable - new collateral falg value for the asset
@external
func modify_core_settings{
    syscall_ptr : felt*,
    pedersen_ptr : HashBuiltin*,
    range_check_ptr
}(id: felt, short_name: felt, tradable: felt, collateral: felt) :
    alloc_locals
    # Auth Check
    let (caller) = get_caller_address()
    let (auth_addr) = auth_address.read()
    ...
    tick_size = _asset.tick_size, step_size = _asset.step_size, minimum_order_size = _asset.minimum_order_size, 
...
func modify_trade_settings{
    ...
    pedersen_ptr : HashBuiltin*,
    range_check_ptr
}(id: felt, tick_size: felt, step_size: felt, minimum_order_size: felt, minimum_leverage: felt, maximum_leverage: felt, 
currently_allowed_leverage: felt, maintenance_margin_fraction: felt, initial_margin_fraction: felt, incremental_initial_margin_fraction: felt, 
incremental_position_size: felt, baseline_position_size: felt, maximum_position_size: felt) :
    alloc_locals
    # Auth Check
    let (caller) = get_caller_address()
    let (auth_addr) = auth_address.read()
    ...
    collateral = _asset.collateral, tick_size = tick_size, step_size = step_size, minimum_order_size = minimum_order_size, 
