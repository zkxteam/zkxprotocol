...
namespace Math64x61:
    ...
    func assert_64x61 {range_check_ptr} (x : felt):
        assert_in_range(x, -BOUND, BOUND)
        return ()
    end

    # Validates that X is a positive Math64x61 value. X must be >= 0.000'000'000'000'000'001
    func assert_positive_64x61 {range_check_ptr} (x : felt):
        assert_in_range(x, MIN_FRACTION, BOUND)
        return ()
    end
    ...
    # Convenience addition method to assert no overflow before return
    ...
    # Convenience subtraction method to assert no overflow before return
    ...
    # Multiplies two fixed point values and checks for overflow before return
    ...
    func exp2 {range_check_ptr} (x : felt) -> (res : felt):
        ...
        let (int_res) = pow_int(2 * ONE, int_part)
