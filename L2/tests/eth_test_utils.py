# this script is driectly derived form starkware.eth.eth_test_utils
# the only change is that the command line args for gaanche-cli have been updated
import contextlib
import logging
import os
import random
...
class Ganache:
    ...
    def __init__(self):
        ...
        self.ganache_proc = subprocess.Popen(
            f"ganache-cli -p {self.port} --chain.chainId 1337 --miner.blockGasLimit 800000000 "
        ...
        self.w3 = Web3(
            HTTPProvider(f"http://127.0.0.1:{self.port}/", request_kwargs=request_kwargs)
        )
