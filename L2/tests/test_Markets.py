...
async def test_add_new_market_invalid_leverage(adminAuth_factory):
    ...
    await assert_revert(signer1.send_transaction(admin1, market.contract_address, 'add_market', [
        DEFAULT_MARKET_ID, str_to_felt("32f0406jz7qj8"), str_to_felt("32f0406jz7qj7"), 1, 0, 60, 1, 0, 1, 0, 10, to64x61(1), to64x61(1), to64x61(3), 5, 1, 1, 100, 1000, 10000] + prepare_starknet_string(DEFAULT_LINK_1)),
...
async def test_add_new_market(adminAuth_factory):
    ...
    await signer1.send_transaction(admin1, market.contract_address, 'add_market', [
        DEFAULT_MARKET_ID, str_to_felt("32f0406jz7qj8"), str_to_felt("32f0406jz7qj7"), 1, 0, 60, to64x61(0.1), 0, to64x61(0.01), 0, 10, to64x61(1), to64x61(5), to64x61(3), 1, 1, 1, 100, 1000, 10000] + prepare_starknet_string(DEFAULT_LINK_1))
...
async def test_get_all_archived_tradable_markets(adminAuth_factory):
    ...
    assert len(list(markets_new.result.array_list)) == len(
        list(markets.result.array_list)) - 6
...
async def test_modifying_trade_settings_by_admin(adminAuth_factory):
    ...
    await signer1.send_transaction(admin1, market.contract_address, 'modify_trade_settings', [
        market_id,
        new_tick_size,
        new_step_size,
        new_ttl,
