...
async def test_fund_reject(holding_factory):
    ...
    assert_revert(lambda: signer3.send_transaction(pytest.user1,holding.contract_address, 'fund', [ str_to_felt("USDC"), 100]))
...
async def test_defund_reject(holding_factory):
    ...
    assert_revert(lambda: signer3.send_transaction(pytest.user1,holding.contract_address, 'defund', [ str_to_felt("USDC"), 100]))