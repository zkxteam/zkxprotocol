...
async def adminAuth_factory():
    ...
    registry.contract_address, 'update_contract_registry', [22, 1, pubkey_whitelister.contract_address])
    await signer1.send_transaction(admin1, 
    registry.contract_address, 'update_contract_registry', [23, 1, sig_req_manager.contract_address])

    await signer1.send_transaction(admin1, 
    registry.contract_address, 'update_contract_registry', [1, 1, test_asset.contract_address])

    return admin1, admin2, user3, user4, registry, test_asset, validator_router, sig_req_manager, pubkey_whitelister

@pytest.mark.asyncio
async def test_unauthorised_whitelist(adminAuth_factory):

    admin1, admin2, user3, user4, registry, test_asset, validator_router, sig_req_manager, pubkey_whitelister = adminAuth_factory

    await assert_revert(signer3.send_transaction(user3, 
...
async def test_simple_call_flow(adminAuth_factory):
    ...
    await signer1.send_transaction(admin1, sig_req_manager.contract_address, 'set_sig_requirement',[
        1,
        1,
        get_selector_from_name('set_asset_value'),
        2
    ...
    await signer1.send_transaction(admin1, pubkey_whitelister.contract_address, 'whitelist_pubkey', [
    ...
    await signer1.send_transaction(admin1, validator_router.contract_address, 'toggle_check_sig', []
    )
    ...
    print(' prev asset value:'+str(current_asset_value))
    ...
    await signer1.send_transaction(admin1, validator_router.contract_address, 'call_core_function', 
    [
        1,
        1,
        current_nonce,
    ...
    print('new_asset_value:'+str(current_asset_value))
...
async def test_call_with_incorrect_nonce(adminAuth_factory):
    ...
    await assert_revert(signer1.send_transaction(admin1, validator_router.contract_address, 'call_core_function', 
    [
        1,
        1,
        0,
...
async def test_call_with_incorrect_signature(adminAuth_factory):
    ...
    [
        ...
        get_selector_from_name('set_asset_value'),
        1,
        30, # signed data was 20 - data tampering should revert call due to signature mismatch
        1,
        user3_sig[0],
        user3_sig[1],
        1,
...
async def test_call_with_0_sig(adminAuth_factory):
    ...
    [
        ...
        40,
        1,
        user4_sig[0], # sig and pubkey mismatch will also work since there is 0 sig requirement
        user4_sig[1], #
        1,
        signer3.public_key,
    ]
...
async def test_call_with_delisted_pubkey(adminAuth_factory):
    ...
    await signer1.send_transaction(admin1, pubkey_whitelister.contract_address, 'delist_pubkey', [
...
async def test_with_deregistered_func(adminAuth_factory):
    ...
    await signer1.send_transaction(admin1, sig_req_manager.contract_address, 'deregister_func', [
        1,
        1,
        get_selector_from_name('set_asset_value')
    ])
