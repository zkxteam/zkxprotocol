...
from utils_trading import User, order_direction, order_types, OrderExecutor, User, ABR, fund_mapping, set_balance, execute_and_compare, compare_fund_balances, compare_user_balances, compare_user_positions, compare_abr_values, check_batch_status, set_abr_value, make_abr_payments
...
async def abr_factory(starknet_service: StarknetService):
    ...
    relay_abr_core = await starknet_service.deploy(ContractType.RelayABRCore, [
...
async def test_view_functions_state_0(abr_factory):
    ...
    no_of_batches_query = await relay_abr.get_no_of_batches_for_current_epoch().call()
...
async def test_set_invalid_timestamp(abr_factory):
    ...
    await assert_revert(
        admin1_signer.send_transaction(
            admin1, relay_abr.contract_address, 'set_abr_timestamp', [timestamp - 1]),
...
async def test_set_timestamp(abr_factory):
    ...
    set_abr_timestamp_tx = await admin1_signer.send_transaction(
        admin1, relay_abr.contract_address, 'set_abr_timestamp', [timestamp_1])
...
async def test_set_abr_1(abr_factory):
    starknet_service, non_admin, admin1, trading, fixed_math, alice,  bob, charlie, dave, abr_calculations, relay_abr, abr_core, abr_fund, abr_payment, timestamp, admin2, alice_test, bob_test, charlie_test, dave_test, python_executor, abr_executor = abr_factory
    # Set BTC_USD ABR
    (set_abr_value_tx, abr_value, abr_last_price) = await set_abr_value(market_id=BTC_USD_ID, node_signer=admin1_signer, node=admin1, abr_core=relay_abr, abr_executor=abr_executor, timestamp=timestamp_1, spot=ABR_data.btc_usd_perp_spot_1, perp=ABR_data.btc_usd_perp_1, spot_64x61=convertTo64x61(ABR_data.btc_usd_perp_spot_1), perp_64x61=convertTo64x61(ABR_data.btc_usd_perp_1), epoch=1, base_rate=0.0000125, boll_width=2.0)
...
async def test_set_abr_2(abr_factory):
    starknet_service, non_admin, admin1, trading, fixed_math, alice,  bob, charlie, dave, abr_calculations, relay_abr, abr_core, abr_fund, abr_payment, timestamp, admin2, alice_test, bob_test, charlie_test, dave_test, python_executor, abr_executor = abr_factory
    # Set ETH_USD ABR
    (set_abr_value_tx, abr_value, abr_last_price) = await set_abr_value(market_id=ETH_USD_ID, node_signer=admin1_signer, node=admin1, abr_core=relay_abr, abr_executor=abr_executor, timestamp=timestamp_1, spot=ABR_data.eth_usd_perp_spot_1, perp=ABR_data.eth_usd_perp_1, spot_64x61=convertTo64x61(ABR_data.eth_usd_perp_spot_1), perp_64x61=convertTo64x61(ABR_data.eth_usd_perp_1), epoch=1, base_rate=0.0000125, boll_width=2.0)
...
async def test_set_timestamp_state_1(abr_factory):
    ...
    await assert_revert(
        admin1_signer.send_transaction(
            admin1, relay_abr.contract_address, 'set_abr_timestamp', [timestamp_2]),
        "ABRCore: Invalid State"
    )
...
async def test_set_abr_3(abr_factory):
    starknet_service, non_admin, admin1, trading, fixed_math, alice,  bob, charlie, dave, abr_calculations, relay_abr, abr_core, abr_fund, abr_payment, timestamp, admin2, alice_test, bob_test, charlie_test, dave_test, python_executor, abr_executor = abr_factory
    # Set BTC_UST ABR
    (set_abr_value_tx, abr_value, abr_last_price) = await set_abr_value(market_id=BTC_UST_ID, node_signer=admin1_signer, node=admin1, abr_core=relay_abr, abr_executor=abr_executor, timestamp=timestamp_1, spot=ABR_data.btc_ust_perp_spot_1, perp=ABR_data.btc_ust_perp_1, spot_64x61=convertTo64x61(ABR_data.btc_ust_perp_spot_1), perp_64x61=convertTo64x61(ABR_data.btc_ust_perp_1), epoch=1, base_rate=0.0000125, boll_width=2.0)
...
async def test_pay_abr_call_1(abr_factory):
    ...
    abr_tx = await make_abr_payments(admin_signer=admin1_signer, admin=admin1, abr_core=relay_abr,
    ...
    remaining_pay_abr_calls_query = await relay_abr.get_remaining_pay_abr_calls().call()
...
async def test_pay_abr_call_2(abr_factory):
    ...
    await make_abr_payments(admin_signer=admin1_signer, admin=admin1, abr_core=relay_abr,
...
async def test_set_timestamp_invalid_epoch_2(abr_factory):
    ...
    await assert_revert(admin1_signer.send_transaction(
        admin1, relay_abr.contract_address, 'set_abr_timestamp', [timestamp_3]),
        "ABRCore: New Timstamp must be > last timestamp + abr_interval"
    )
...
async def test_set_timestamp_epoch_2(abr_factory):
    ...
    set_abr_timestamp_tx = await admin1_signer.send_transaction(
        admin1, relay_abr.contract_address, 'set_abr_timestamp', [timestamp_4])
...
async def test_set_abr_round_2(abr_factory):
    ...
    # Set BTC_UST ABR
    (set_abr_value_tx, abr_value, abr_last_price) = await set_abr_value(market_id=BTC_UST_ID, node_signer=admin1_signer, node=admin1, abr_core=relay_abr, abr_executor=abr_executor, timestamp=timestamp_4, spot=ABR_data.btc_ust_perp_spot_2, perp=ABR_data.btc_ust_perp_2, spot_64x61=convertTo64x61(ABR_data.btc_ust_perp_spot_2), perp_64x61=convertTo64x61(ABR_data.btc_ust_perp_2), epoch=2, base_rate=0.000025, boll_width=1.5)
    ...
    # Set BTC_USD ABR
    (set_abr_value_tx, abr_value, abr_last_price) = await set_abr_value(market_id=BTC_USD_ID, node_signer=admin1_signer, node=admin1, abr_core=relay_abr, abr_executor=abr_executor, timestamp=timestamp_4, spot=ABR_data.btc_usd_perp_spot_2, perp=ABR_data.btc_usd_perp_2, spot_64x61=convertTo64x61(ABR_data.btc_usd_perp_spot_2), perp_64x61=convertTo64x61(ABR_data.btc_usd_perp_2), epoch=2, base_rate=0.000025, boll_width=1.5)
    ...
    # Set ETH_USD ABR
    (set_abr_value_tx, abr_value, abr_last_price) = await set_abr_value(market_id=ETH_USD_ID, node_signer=admin1_signer, node=admin1, abr_core=relay_abr, abr_executor=abr_executor, timestamp=timestamp_4, spot=ABR_data.eth_usd_perp_spot_2, perp=ABR_data.eth_usd_perp_2, spot_64x61=convertTo64x61(ABR_data.eth_usd_perp_spot_2), perp_64x61=convertTo64x61(ABR_data.eth_usd_perp_2), epoch=2, base_rate=0.000025, boll_width=1.5)
