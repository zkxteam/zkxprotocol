...
@view 
func check{range_check_ptr} (x: felt, y: felt) -> (res: felt):
    let (two_percent) = mul_fp(x, 46116860184273879)
    
    tempvar lowerLimit = x - two_percent
    tempvar upperLimit = x + two_percent
    # assert_le(y, upperLimit)
    # assert_le(lowerLimit, y)
    assert_in_range(y, lowerLimit, upperLimit)
    return (two_percent)
end